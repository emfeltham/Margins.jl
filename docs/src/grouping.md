...
policy_scenarios = population_margins(model, data;
                                    type=:effects,
                                    scenarios=Dict(:policy_level => ["none", "moderate", "aggressive"]))
```
...
healthcare_comprehensive = population_margins(health_model, health_data;
    ...
    scenarios=Dict(:aca_expansion => [0, 1], :medicaid_funding => [0.8, 1.2])
)
...
tax_analysis = population_margins(tax_model, tax_data;
    ...
    scenarios=Dict(:tax_rate => [0.15, 0.25, 0.35], :deduction_cap => [10000, 25000])
)
...
labor_analysis = population_margins(employment_model, labor_data;
    ...
    scenarios=Dict(:unemployment_rate => [3.0, 6.0, 9.0])  # Economic conditions
)
```
...
clinical_analysis = population_margins(treatment_model, patient_data;
    ...
    scenarios=Dict(:treatment_protocol => ["standard", "intensive"],
...
education_policy = population_margins(achievement_model, student_data;
    ...
    scenarios=Dict(:intervention => [0, 1], :funding_increase => [0.0, 0.1, 0.2])
)
```
...
@btime population_margins($model, $data; groups=:education, scenarios=Dict(:treatment => [0, 1]))
```
...
research_analysis = population_margins(model, data;
                                     groups=:region => [:education, (:income, 4)],
                                     scenarios=Dict(:policy => [0, 1], :funding => [0.8, 1.2]))
```
