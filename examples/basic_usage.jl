...
println("• profile_margins(model, data, reference_grid; type, vars, ...) - profile-based analysis")
println()
println("Parameters:")
println("• type = :effects (derivatives) or :predictions (levels)")
println("• vars = variables for effects (e.g., [:x1, :x2] or :all_continuous)")
println("• scale = :response or :link (for both effects and predictions)")
println("• Profile grids: means_grid(data), cartesian_grid(vars...), balanced_grid(data; vars...)")
println("• Population scenarios: scenarios=(...,) for counterfactuals")
println()
