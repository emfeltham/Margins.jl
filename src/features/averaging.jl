...
"""
function _average_rows_with_proper_se(df, G::Matrix{Float64}, Σ::AbstractMatrix, group_cols::Vector{String})
...
"""
function _average_profiles_with_proper_se(df, gradients::Dict, Σ::AbstractMatrix, group_cols::Vector{String})
    profile_cols = [c for c in names(df) if startswith(String(c), "at_")]
