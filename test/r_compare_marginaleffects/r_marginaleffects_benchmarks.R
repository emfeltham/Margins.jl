# r_marginaleffects_benchmarks.R
# R performance benchmarks using marginaleffects package (state-of-the-art)
# For comparison with Julia Margins.jl
# Uses microbenchmark package for accurate timing and profmem for memory tracking
...
cat("================================================================================\n")
cat("R PERFORMANCE BENCHMARK with marginaleffects (Speed & Memory)\n")
cat("================================================================================\n\n")
...
cat("================================================================================\n")
cat("RUNNING BENCHMARKS with marginaleffects\n")
cat("================================================================================\n\n")
...
# marginaleffects uses slopes() with newdata parameter for profile-based marginal effects
cat("2. MEM (Marginal Effects at Profiles - slopes at grid)\n")
...
mem <- microbenchmark(
  {
    # Compute marginal effect for each continuous variable at the reference grid
    for (var in continuous_vars) {
      slopes(model_r,
             variables = var,
             newdata = datagrid(socio4 = c(FALSE, TRUE),
                               are_related_dists_a_inv = c(1.0, 1/6)),
             type = "response")
    }
  },
...
cat("   Note: slopes() computes derivatives at grid points, matching Julia\n\n")
...
ame_age <- microbenchmark(
  slopes(model_r, variables = "age_h"),
  times = 5
)
...
ame_scenario <- microbenchmark(
  slopes(
    ...
    newdata = datagrid(are_related_dists_a_inv = 1/6)
  ),
  times = 5
)
...
saveRDS(benchmarks, "r_marginaleffects_benchmarks.rds")

cat("================================================================================\n")
cat("RESULTS SAVED\n")
cat("================================================================================\n\n")
cat("✓ r_marginaleffects_benchmarks.rds\n\n")
...
cat("Total operation time:",
    ...
    "s (R marginaleffects)\n\n")
cat("Next: Compare with Julia using compare_performance_marginaleffects.jl\n\n")
