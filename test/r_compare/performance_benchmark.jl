...
# PERFORMANCE-FOCUSED comparison: Julia vs R margins package
#
# Focus: Speed and memory usage only (correctness validation done separately)
# Run both this and r_model.R, then compare_performance.jl to see speedup ratios

using Margins, GLM, DataFrames, CSV, CategoricalArrays
using BenchmarkTools, Statistics

println("=" ^ 80)
println("JULIA PERFORMANCE BENCHMARK (Speed & Memory)")
println("=" ^ 80)
println()
...
df = CSV.read(joinpath(script_dir, "r_comparison_data.csv"), DataFrame)
N = nrow(df)
...
# Fit model (same as julia_model.jl)
...
# Save results
CSV.write(joinpath(script_dir, "julia_benchmarks.csv"), results)
println("=" ^ 80)
println("RESULTS SAVED")
println("=" ^ 80)
println()
println("✓ julia_benchmarks.csv")
println()
...
println("Total benchmark time: $(round(sum(results.time_s), digits=2))s (Julia)")
println()
println("Next: Compare with R using compare_performance.jl")
println()
