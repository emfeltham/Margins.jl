...
@testset "Scenario 1: No interactions - continuous predictors" begin
    ...
    ame_sw = ame(m, :SepalWidth, iris)
    ame_pl = ame(m, :PetalLength, iris)
    ame_pw = ame(m, :PetalWidth, iris)
    ame_all = ame(m, [:SepalWidth, :PetalLength, :PetalWidth], iris)

    @test isapprox(ame_sw.ame[:SepalWidth], coefs[idx_sw]; atol=1e-8)
    @test isapprox(ame_sw.se[:SepalWidth], ses[idx_sw]; atol=1e-8)
    @test isapprox(ame_pl.ame[:PetalLength], coefs[idx_pl]; atol=1e-8)
    @test isapprox(ame_pl.se[:PetalLength], ses[idx_pl]; atol=1e-8)
    @test isapprox(ame_pw.ame[:PetalWidth], coefs[idx_pw]; atol=1e-8)
    @test isapprox(ame_pw.se[:PetalWidth], ses[idx_pw]; atol=1e-8)
    @test all(isapprox.(ame_all.ame[:SepalWidth], coefs[idx_sw]; atol=1e-8))
end
...
@testset "Scenario 2: Interaction" begin
    ...
    @test isapprox(ame2.ame[:SepalWidth], ame_closed; atol=1e-8)
    @test isapprox(ame2.se[:SepalWidth], se_closed; atol=1e-8)
end
...
@testset "Scenario 3: Transformation of predictor" begin
    ...
    @test isapprox(ame3.ame[:SepalWidth], ame_closed3; atol=1e-8)
    @test isapprox(ame3.se[:SepalWidth], se_closed3; atol=1e-8)
end
...
@testset "Scenario 4: Representative values of moderator" begin
    ...
    for pw in quantiles_pw
        ame_val = ame4.ame[:SepalWidth][(pw,)]
...
@testset "Scenario 5: Interaction with categorical moderator" begin
    ...
    ame5 = ame(m5, :SepalWidth, iris; repvals=repvals_cat)
    ...
    for lvl in levels_list
        ...
        ame_val = ame5.ame[:SepalWidth][(lvl,)]
        se_val = ame5.se[:SepalWidth][(lvl,)]
