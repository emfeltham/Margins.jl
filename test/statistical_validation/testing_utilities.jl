# testing_utilities.jl
# Testing utilities for statistical validation framework
#
...
function make_heteroskedastic_data(; n=500, heteroskedasticity_type=:linear, seed=42)
    ...
    # Generate heteroskedastic errors following proper statistical practice
    if heteroskedasticity_type == :linear
        # Strong linear heteroskedasticity: variance increases with |x|
        # Following user's better approach: ϵ ~ N(0, (0.5 + |x|)²)
        errors = randn(n) .* (0.5 .+ abs.(df.x))
    elseif heteroskedasticity_type == :quadratic
        # Strong quadratic heteroskedasticity: variance increases with x²
        errors = randn(n) .* (0.3 .+ 0.5 .* abs.(df.x).^1.5)
